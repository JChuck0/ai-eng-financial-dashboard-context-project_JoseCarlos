...
export function IncomeOutcomeChart({ data, loading }: IncomeOutcomeChartProps) {
  if (loading) {
    return (
      <Card className="border-border/60" aria-busy="true" aria-label="Loading income and outcome chart">
  ...
  const totalOutcome = data.reduce((total, item) => total + item.outcome, 0)
        ...
        {!hasData ? (
          ...
          <figure>
            <figcaption className="sr-only">
              Income and outcome chart across {data.length} monthly periods. Total income is{' '}
              {formatCurrency(totalIncome)} and total outcome is {formatCurrency(totalOutcome)}.
            </figcaption>
